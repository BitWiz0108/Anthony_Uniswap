...
export class XDefiWallet {
  constructor(chain, xDefiObject, address) {
    this.chain = chain;
  ...
  async depositThorchain({ inbounds, amount, memo }) {
    const inbound = inbounds.find((i) => i.chain === this.chain);
  ...
  async depositEthereum({ inbounds, amount, memo, asset = "" }) {
    ...
    let tokenContract = (asset.split("-")[1] || "").toLowerCase();
    if (!tokenContract) {
      tokenContract = "0x0000000000000000000000000000000000000000";
    }
    const decimals = TOKENS_DECIMALS[tokenContract] || 18;
    const n = parseUnits(amount, decimals);
    const tx = await contract.deposit(inbound.address, tokenContract, n, memo, {
      value: asset === "ETH.ETH" ? n : 0,
    });
    return tx.hash;
  }
  ...
  async transfer({ to, amount, asset }) {
    switch (this.chain) {
      case "MAYA":
        return this.transferCosmos({ to, amount, asset });
      case "THOR":
        return this.transferThorchain({ to, amount});
      case "ETH":
        return this.transferEthereum({ to, amount, asset });
      case "BTC":
        return this.transferBitcoin({ to, amount, asset });
  ...
  async transferThorchain({ to, amount, memo = "" }) {
    let asset = {
      chain: "THOR",
      symbol: "RUNE",
      ticker: "RUNE",
    ...
    let amountStr = (parseFloat(amount) * 1e8).toFixed(0)
    let denomAmount = {
      amount: amountStr,
      decimals: 8
    };
  ...
  async transferEthereum({ to, amount, asset }) {
    ...
    const decimals = await contract.decimals();
    const n = parseUnits(amount, decimals);
    const tx = await tokenContract.transfer(to, n);
    return tx.hash;
  }
